...
import {
  Box, Button, Popover, TextField, Typography, CircularProgress, InputAdornment,
} from '@mui/material';
import {
  SearchOutlined, KeyboardArrowDownOutlined, CheckOutlined,
...
export default function ModelPicker({ value, onChange, filter, placeholder = 'Select model' }: ModelPickerProps) {
  ...
  const grouped = useMemo(() => {
    ...
    const filtered = q
      ? base.filter((m) => m.id.toLowerCase().includes(q) || (m.provider ?? '').toLowerCase().includes(q))
      : base;
  ...
  const close = () => { setAnchorEl(null); setQuery(''); };
    ...
    <>
      <Button
        ...
        startIcon={
          loading ? (
            ...
            <Box sx={{ width: 6, height: 6, borderRadius: '50%', bgcolor: 'success.main', boxShadow: '0 0 8px rgba(34,197,94,0.6)' }} />
          )
        }
      ...
      >
        <Box sx={{ p: 1.5, borderBottom: '1px solid rgba(161,161,170,0.08)' }}>
          <TextField
            autoFocus fullWidth size="small" placeholder="Search models…"
        ...
        <Box sx={{ overflowY: 'auto', maxHeight: 460 }}>
          ...
          {!loading && grouped.map(([provider, items]) => (
            <Box key={provider}>
              <Box sx={{ px: 2, py: 1, position: 'sticky', top: 0, background: 'rgba(20,20,23,0.96)', backdropFilter: 'blur(8px)', borderBottom: '1px solid rgba(161,161,170,0.05)' }}>
                <Typography variant="caption" sx={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'text.secondary' }}>
              ...
              {items.map((m) => {
                ...
                return (
                  <Box
                    ...
                    sx={{
                      px: 2, py: 0.85, cursor: 'pointer', display: 'flex', alignItems: 'center',
                      justifyContent: 'space-between', gap: 1.5,
                      background: selected ? 'rgba(139,92,246,0.12)' : 'transparent',
                      borderLeft: '2px solid', borderColor: selected ? 'primary.light' : 'transparent',
                      '&:hover': { background: selected ? 'rgba(139,92,246,0.15)' : 'rgba(161,161,170,0.05)' },
                      transition: 'background 0.1s',
                    }}
                  >
                    <Typography sx={{ fontSize: 12.5, fontFamily: '"JetBrains Mono","SF Mono",monospace', color: 'text.primary', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>
                      {m.id}
                    </Typography>
