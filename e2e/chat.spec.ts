...
test.describe("Chat flow", () => {
  ...
  test("empty hero + suggestion cards", async ({ page }, info) => {
    ...
    await expect(
      page.getByText("Explain a concept", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Write something", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Debug my code", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Brainstorm ideas", { exact: true }),
    ).toBeVisible();
  ...
  test("sends a message and receives a streamed response", async ({ page }) => {
    ...
    await expect(
      page.getByText("Say the single word: pong", { exact: true }),
  ...
  test("footer shows the active model", async ({ page }) => {
    ...
    await expect(
      page.getByText(new RegExp(`Flux AI · ${E2E.model}`)),
    ).toBeVisible();
  });
});
